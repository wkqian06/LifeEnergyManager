...
If the user provides extra tasks, call `UrgencyTriageAgent` when subagent tools are available. If unavailable, record `UrgencyTriageAgent: unavailable fallback` and complete the same triage in the main thread. The main thread must accept or reject each extra task.
...
Call `DailyPlannerAgent` when subagent tools are available to draft plan options. Call `AdviceAgent` when subagent tools are available to draft the status summary, today advice, and anti-distraction tip. If either agent is unavailable, record `unavailable fallback` and complete the same structured pass in the main thread. The main thread must choose the final plan.
...
After generating artifacts, call `ArtifactQAAgent` when subagent tools are available. If unavailable, record `ArtifactQAAgent: unavailable fallback` and complete the same QA checklist in the main thread. Fix any issues before presenting artifacts.
...
```text
Subagent calls:
- UrgencyTriageAgent: used / not needed / unavailable fallback
- DailyPlannerAgent: used / not needed / unavailable fallback
- AdviceAgent: used / not needed / unavailable fallback
- ArtifactQAAgent: used / not needed / unavailable fallback
- Reason:
