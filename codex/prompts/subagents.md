...
- Evening: use `$life-energy-drive-resistance` when enough report content exists to produce the three daily metrics (energy reserve, predicted next-day drive, actual drive); escalate to `EnergyQuantAgent` when the report is ambiguous, its signals diverge, or the result would change next-day intensity.
