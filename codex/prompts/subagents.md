...
- Evening: use `$life-energy-drive-resistance` when enough report content exists to produce the three daily metrics (energy remaining, predicted next-day drive, actual drive (night summary)); escalate to `EnergyQuantAgent` when the report is ambiguous, its signals diverge, or the result would change next-day intensity.
...
- Produce the three daily scoring metrics (energy remaining, predicted next-day drive, actual drive (night summary)) from the evening report. Definitions live in the tracker's Daily Scoring Model; all are 0-100, higher = better.
...
- Blind pass first, from report evidence only: `remainingBlind`, `predDriveBlind`, `actualDrive` (anchor actual drive (night summary) on focus minutes and completions). Then read the user self-scores (`remainingSelf`, `predDriveSelf`) and produce the calibrated values; blind values are never edited.
- `planning_adjustment` is informed by energy remaining and actual drive (night summary); the predicted-vs-actual comparison is calibration only, not a planning input.
