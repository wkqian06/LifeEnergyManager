...
- run $life-energy-goal-drift-guard before intake; block on due goals until I choose a terminal outcome,
...
- use $life-energy-plan-revision when my input may change commitments, week, month, phase, scope, sequence, or deadlines,
- allow inline changes only inside existing capacity; otherwise enter the pre-artifact correction mode from morning.md,
- require three separate replies for month/phase changes and every rebaseline, then explicitly exit correction mode and reread the confirmed plans,
...
- atomically persist `outputs/artifact-locks/YYYY-MM-DD.json` before either renderer starts; treat it as the same-day revision lock even after interruption and never revise/regenerate today's pair,
...
Read codex/prompts/subagents.md first. Immediately ask me to paste the report generated by today's HTML workbench and wait for it; do not scan outputs/ for an existing report. Follow codex/prompts/evening.md exactly:
...
- update Planning Calibration from the structured task estimate samples, critical-path minutes, weekly output completion rate, and unplanned-work fields; run $life-energy-goal-drift-guard for goal closure,
- save any standalone daily report under outputs/daily-reports/,
- produce the three daily metrics (energy remaining, predicted next-day drive, actual drive (night summary)) per the tracker Daily Scoring Model using $life-energy-drive-resistance: blind pass first, then read my self-scores and calibrate; actual drive (night summary) is a single blind value,
...
- use $life-energy-goal-drift-guard to force terminal outcomes for due goals and audit proximity, cumulative delay, revisions, and goal debt,
