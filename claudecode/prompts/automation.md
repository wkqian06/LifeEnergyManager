...
- require three separate replies for month/phase changes and every rebaseline, then explicitly exit correction mode and reread confirmed plans,
...
Read claudecode/prompts/subagents.md first. Immediately ask me to paste the report generated by today's HTML workbench and wait for it; do not scan outputs/ for an existing report. Follow claudecode/prompts/evening.md exactly:
...
- update Planning Calibration from structured task estimate samples, critical-path minutes, weekly output completion rate, and unplanned-work fields; run life-energy-goal-drift-guard for goal closure,
