...
description: Produce LifeEnergyManager's three daily scoring metrics (energy reserve, predicted next-day drive, actual drive) from evening reports, with blind and calibrated agent values plus a conservative planning adjustment. Use as the default bounded-analysis path before any justified energy-quant subagent escalation.
---
...
- The user self-scores (`reserveSelf`, `predDriveSelf`) and notes are NOT inputs to the blind pass. Do not read them before the blind estimate; they are the user's own independent evening evaluation, weighed only at the calibration step.

## Procedure

1. Separate evidence from inference.
2. **Blind pass** (user self-scores not read), from report evidence only:
   - `reserveBlind` - energy left after today.
   - `predDriveBlind` - drive expected tomorrow.
   - `actualDrive` - drive actually available today (single value, anchored on focus minutes and completions; never calibrated).
3. Set `agent_energy_confidence` (low / medium / high) and write a short summary.
4. Read the user self-scores and notes, then produce `reserveCalibrated` and `predDriveCalibrated` by weighing them. The blind values are never edited.
5. Recommend `planning_adjustment` from today's energy reserve and actual drive (they feed tomorrow's sizing), without punishment or shame. Predicted drive is a forecast used for calibration, not a planning input.
...
(The user self-scores `reserveSelf` / `predDriveSelf` come from the workbench, not from this skill.)
