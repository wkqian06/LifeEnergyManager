...
description: Produce LifeEnergyManager's three daily scoring metrics (energy remaining, predicted next-day drive, actual start-of-day drive) from evening reports, with blind and calibrated agent values plus a conservative planning adjustment. Use as the default bounded-analysis path before any justified energy-quant subagent escalation.
---
...
2. **Blind pass** (user self-scores not read), from report evidence only:
   ...
   - `actualDrive` - the drive/energy you actually had starting the day, how easy it was to get going (single value, anchored on focus minutes and completions; never calibrated).
...
5. Recommend `planning_adjustment` from today's energy remaining and actual start-of-day drive (they feed tomorrow's sizing), without punishment or shame. Predicted drive is a forecast used for calibration, not a planning input.
