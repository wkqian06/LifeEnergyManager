...
description: Produce LifeEnergyManager's three daily scoring metrics (energy remaining, predicted next-day drive, actual drive (night summary)) from evening reports, with blind and calibrated agent values plus a conservative planning adjustment. Use as the default bounded-analysis path before any justified energy-quant subagent escalation.
---
...
5. Recommend `planning_adjustment` from today's energy remaining and actual drive (night summary) (they feed tomorrow's sizing), without punishment or shame. Predicted drive is a forecast used for calibration, not a planning input.
