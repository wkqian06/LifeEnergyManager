...
description: Classify extra LifeEnergyManager morning tasks as critical, goal-leveraged, maintenance, or distraction, and propose time caps and tradeoffs. Use as the default bounded-analysis path before any justified urgency-triage subagent escalation.
---
...
Use this skill as the default bounded-analysis contract for extra urgent, external, or tempting tasks before the main session accepts or rejects them. Escalate to the `urgency-triage` subagent only when a task would displace thesis-critical work, has ambiguous urgency, or looks like productive procrastination.
...
- The main session must make final tradeoff decisions.
