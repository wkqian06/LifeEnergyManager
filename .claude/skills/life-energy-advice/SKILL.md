...
description: Draft concise LifeEnergyManager status summary, today advice, and anti-distraction tip from rolling state and daily plan context. Use as the default bounded-analysis path before any justified advice subagent escalation.
---
...
Use this skill as the default bounded-analysis contract for short daily reminder text. Escalate to the `advice` subagent only when state interpretation or the distraction pattern is unclear from evidence. It drafts short reminder text for the provisional plan and wallpaper right column.
...
- The main session may edit the final wording.
