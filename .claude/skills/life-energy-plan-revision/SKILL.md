...
monthly gate, or phase target. It proposes a bounded change set; the main session
owns mode entry, confirmations, writes, rollback, and the final daily plan.

Escalate to the `plan-revision` subagent only for month/phase changes,
rebaseline, red feasibility, or conflicting plan sources.
