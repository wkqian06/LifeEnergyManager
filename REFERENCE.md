...
flowchart TB
    ...
    subgraph daily["The daily rhythm · Monday–Saturday"]
        ...
        reflect["🌙 Reflect<br/>👤 you report how it went<br/>captures progress &amp; blockers<br/>scores energy reserve and next-day drive (0–100)"]
        plan --> run --> reflect
    end
...
- Consider yesterday's energy reserve and actual drive (agent-calibrated variant as the primary signal) when choosing intensity.
...
- Score the three daily metrics with the drive-resistance skill, all 0-100 and higher = better (see the Daily Scoring Model in `templates/tracker.md`): energy reserve, predicted next-day drive, and actual drive. The agent scores blind first, then reads the user self-scores and calibrates; actual drive is a single blind value. Energy reserve and actual drive inform the next day's sizing; the predicted-vs-actual comparison is recorded for calibration only. Not a diagnosis.
