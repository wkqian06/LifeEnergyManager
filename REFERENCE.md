...
- Consider yesterday's energy remaining and actual start-of-day drive (agent-calibrated variant as the primary signal) when choosing intensity.
...
- Score the three daily metrics with the drive-resistance skill, all 0-100 and higher = better (see the Daily Scoring Model in `templates/tracker.md`): energy remaining, predicted next-day drive, and actual start-of-day drive. The agent scores blind first, then reads the user self-scores and calibrates; actual start-of-day drive is a single blind value. Energy remaining and actual start-of-day drive inform the next day's sizing; the predicted-vs-actual comparison is recorded for calibration only. Not a diagnosis.
